Прогнозування цін: poly_model.predict(X_test_final_combined_for_poly) використовує навчену модель поліноміальної регресії (poly_model) для прогнозування цін на книги, базуючись на попередньо оброблених тестових даних (X_test_final_combined_for_poly). Ці дані вже були масштабовані та закодовані так само, як і тренувальні дані.
Обробка від'ємних значень: np.maximum(0, ...) є функцією з бібліотеки NumPy, яка гарантує, що всі прогнозовані ціни будуть невід'ємними. Оскільки лінійні або поліноміальні моделі можуть іноді генерувати від'ємні значення, цей крок замінює будь-які від'ємні прогнози на 0, оскільки ціна книги не може бути меншою за нуль. Це робить прогнози більш реалістичними.
