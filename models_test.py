...
# Combine scaled numerical features with encoded categorical features for df_test_processed
X_test_final_combined_for_elastic = np.hstack((X_test_processed_scaled_numerical, df_test_processed[categorical_features].values))

# Make predictions on the preprocessed test data
# y_pred_elastic_1 = elastic_net_model.predict(X_test_final_combined_for_elastic)
y_pred_elastic_1 = np.maximum(0, elastic_net_model.predict(X_test_final_combined_for_elastic))

# Оцінка моделі Elastic Net
mae_elastic, mse_elastic, mape_elastic, r2_elastic, accuracy_elastic = evaluate_model(df_sample_submission['Price'], y_pred_elastic_1, "Elastic Net Model for test")

# Графік порівняння реальних цін із прогнозованими Elastic Net Model
plot_predictions(df_sample_submission['Price'], y_pred_elastic_1, "Elastic Net Model for test", 'blue')

# Визначення точок даних з найбільшими помилками прогнозування
display_top_errors(df_sample_submission['Price'], y_pred_elastic_1, "Elastic Net for test")
